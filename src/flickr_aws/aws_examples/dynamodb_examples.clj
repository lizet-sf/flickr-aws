(ns flickr-aws.aws-examples.dynamodb-examples
  (:require [clojure.core.async :as a]
            [clojure.java.io :as io]
            [clojure.data.json :as json]
            [cognitect.aws.client.api :as aws]
            [flickr-aws.ports.aws :as ports.aws]
...
(comment
  ...
  (ports.aws/ddb-batch-put-items ddb
                                 (adapters.aws/->ddb-batch-put-items-request [{:item-key {:partition-key "20250223"
                                                                                          :sort-key "1"}}
                                                                              {:item-key {:partition-key "20250224"
                                                                                          :sort-key "1"}}]))
